...
def transform_name(old_name):
    match = re.match(r"WtoLNu-2Jets_PTLNu-([0-9to-]+)_([0-9]+)J_amcatnloFXFX-pythia8", old_name)
    if match:
        PTLNu, J = match.groups()
        new_name = f"WtoLNu-2Jets_Bin-{J}J-PTLNu-{PTLNu}_amcatnloFXFX-pythia8"
...
def copy_and_rename_files_and_folders(directory):
    ...
    for foldername in os.listdir(directory):
        if "WtoLNu-2Jets_PTLNu" in foldername and "amcatnloFXFX" in foldername:
            ...
            for filename in os.listdir(new_folder_path):
                ...
                if filename.endswith(".json") or filename.endswith(".dat") or filename.endswith(".f"):
                    ...
                    if new_file_name.endswith("proc_card.dat"):
                        try:
                            subprocess.run(
...
if __name__ == "__main__":
    directory = "/afs/cern.ch/user/e/eunsu/private/GridpackFiles/Cards/MadGraph5_aMCatNLO/W"
